...
def process_image(img_path, params):
    try:
        ...
        cv2.imwrite(edge_map_path, (edge_map * 255).astype(np.uint8))
                        ...
                        for scan_date in os.listdir(eye_path):
                            try:
                                if eye_df['Diagnosis Date'].isin([scan_date]).any():
                                    ...
                                    b_scans_path = os.path.join(scan_date_path, "B-Scans")
                                    if os.path.isdir(b_scans_path):
                                        for img_name in os.listdir(b_scans_path):
                                            if img_name.lower().endswith(('.jpg', '.png')):
                                                image_paths.append(os.path.join(b_scans_path, img_name))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--table", type=str, required=True, help="CSV path to patient metadata table")
